...
#     see also: https://developers.yubico.com/Software_Projects/YubiKey_Device_Configuration/
...
# *  Tested and works well with Nerd Fonts. My preference for Gnome is 'Overpass Nerd Font' but this
#    can easily be changed to your own liking. See "FONT" and "LABELFONT" variables below.
#
...
#    key as usual and the value will be recorded to the system clipboard. 
...
FONT="size=11 font='Overpass Nerd Font'"
LABELFONT="size=12 font='Overpass Nerd Font'"
...
# on-click action - copy to clipboard
if [ "$1" = 'copyCode' ]; then
    if [ "$ykPassRequired" = true ]
    then
        ykman oath accounts code -p $ykPassword -s "$2" | xsel --clipboard --input
    else
        ykman oath accounts code -s "$2" | xsel --clipboard --input
    fi
fi
