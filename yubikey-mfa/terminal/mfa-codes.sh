...
# GitHub       : https://github.com/bryandodd/bitbar
###############################################################################
...
getYubikeyOath() {
    ...
    if [[ "$#" == "0" ]]; then
        ...
        if [[ $opt =~ ^[0-9]+$ ]] && (( (opt >= 1) && (opt <= "${ykItemCount}" ) )); then
            ...
            echo "$oathCode" | xsel --clipboard --input
        ...
        if (( (opt >= 1) && (opt <= 32) )); then
            ...
            printf "${redexclaim} YubiKey only supports up to 32 OATH keys. Enter a value from 1 to 32.\n"
        fi
    elif [[ "$#" == "1" ]]; then
